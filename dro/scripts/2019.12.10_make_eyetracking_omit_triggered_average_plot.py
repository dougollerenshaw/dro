...
def make_figure(osid):
    
    eye_data = open_eye_data(osid)
    behavior_data = open_behavior_data(osid)
    response_data = get_response_data(eye_data,behavior_data)

    manifest = pd.read_csv('/allen/programs/braintv/workgroups/nc-ophys/Doug/2019.12.10.sessions_with_tracking.csv')

    session = manifest.query('ophys_session_id == @osid').iloc[0]

    fig2,ax=plt.subplots(3,1,figsize=(8,5),sharex=True)

    plot_event_triggered_response(
        response_data['pupil_area']['omitted_stim'], 
        ax[0], 
        title='pupil area (mean subtracted), osid={}'.format(osid), 
        mean_subtract=True, 
        show_traces=True, 
    ...
    plot_event_triggered_response(
        response_data['running']['omitted_stim'], 
        ax[2], 
        title='running speed, osid={}'.format(osid), 
        mean_subtract=False, 
        show_traces=False, 
        time_zero_subtract = False
    )
    for axis in ax:
        axis.axvline(0,color='red')

    ax[0].set_ylim([-3000,3000])
    ax[1].set_ylim([-1000,1000])

    # ax.plot(df['t'],df[cols].mean(axis=1),color='black',linewidth=3)
    ax[2].set_xlabel('time from event (s)')
    ax[0].set_ylabel('pixels^2')
    ax[1].set_ylabel('pixels^2')
    ax[2].set_ylabel('speed (cm/s)')

    ax[1].set_xlim(-5,5)

    for axis in ax:
        designate_flashes(axis,omit=0)
    sns.despine()
    fig2.tight_layout()

    fig2.subplots_adjust(top=0.80)
    fig2.suptitle('omission triggered average\nsession = {}'.format(session['stage_name']))
...
if __name__ == '__main__':
    ...
    fig = make_figure(osid)
    vbp.save_figure(fig,'/allen/programs/braintv/workgroups/nc-ophys/visual_behavior/eye_tracking_figures/omission_triggered_responses/osid={}'.format(osid))